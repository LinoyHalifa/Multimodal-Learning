...
# Bentzii Add
    ...
    parser.add_argument('--dropout', type=float, default=None, help='dropout') # for cross-corpus test  
    ...
    #Bentzi chage to speedup and utilize gpu
    ...
    print(f"\nBen52: Number of folds: {len(train_loaders)}")
    ...
    if test_loaders:
        for i, test_loader in enumerate(test_loaders):
            print(f"Ben61 Test set {i}: {len(test_loader.dataset)} samples")
    else:
        print("Ben61 No test loaders found.")
    ...
    for ii in range(len(train_loaders)):
       ...
       # Bentzi add to see sampels per folds
        ...
        print(f'Ben71 model: {model}, args: {args}')
        print("ben72",model.model)          # Does it have `.model`?
        print(f'Ben73 model: {model.model}, args: {args}')
