# Bentzi Add to avoid packeges warnings
import warnings
...
# Bentzi Add to replace linux fukenabe chars
...
def train_or_eval_model(args, model, reg_loss, cls_loss, dataloader, epoch, optimizer=None, train=False):
    ...
    for iter, data in enumerate(dataloader):
        ...
        if train and args.savemodel and (iter+1) % args.save_iters == 0: # [time consuming]
            ...
            #Bentzi add to avid not allowed chars in file name
            save_path = clean_filename(save_path)            
...
# Bentzii Add
    ...
    parser.add_argument('--dropout', type=float, default=0.2, help='dropout') # for cross-corpus test  
    ...
    #Bentzi chage batch size to speed up and utilize gpu
    #parser.add_argument('--batch_size', type=int, default=32, metavar='BS', help='batch size [deal with OOM]')
    parser.add_argument('--batch_size', type=int, default=128, metavar='BS', help='batch size [deal with OOM]')
    #Bentzi set to defaolt as 2 
    #parser.add_argument('--num_workers', type=int, default=0, metavar='nw', help='number of workers')
    parser.add_argument('--num_workers', type=int, default=2, metavar='nw', help='number of workers')
    ...
    print ('Ben00====== Params Pre-analysis =======')
    ...
    print(f"\nBen07: Number of train samples in fold: {len(train_loaders)}")
    print(f"\nBen07: Number of eval_loaders samples in fold: {len(eval_loaders)}")
    print(f"\nBen07: Number of test_loaders samples in fold: {len(test_loaders)}")


    print(train_loaders[0].dataset)
    print(eval_loaders[0].dataset)
    print(test_loaders[0].dataset if test_loaders else "No test loaders available")
   



    # for i, (train_loader, eval_loader) in enumerate(zip(train_loaders, eval_loaders)):
    #     print(f"Fold {i}:")
    ...
    if test_loaders:
        for i, test_loader in enumerate(test_loaders):
            print(f"Test set {i}: {len(test_loader.dataset)} samples")
    else:
        print("No test loaders found.")

    # for ii, data in enumerate(train_loaders[0]): break ## debug
    assert len(train_loaders) == len(eval_loaders)
    print (f'Ben03 train&val folder:{len(train_loaders)}; test sets:{len(test_loaders)}')
    args.audio_dim, args.text_dim, args.video_dim = train_loaders[0].dataset.get_featdim()


    print ('03====== Training and Evaluation =======')
    folder_save = [] # 存储每个folder下的最好结果
    folder_duration = []
    for ii in range(len(train_loaders)):
        print('Ben01 args before training: ', args)
        print (f'Ben05>>>>> Cross-validation: training on the {ii+1} folder >>>>>')
        train_loader = train_loaders[ii]
        eval_loader  = eval_loaders[ii]
        start_time = name_time = time.time()
       # Bentzi add to see sampels per folds
        print (f'Ben00 for cv:{ii} train samples :{len(train_loader.sampler)}, eval samples: {len(eval_loader.sampler)}')

        print (f'Step1: build model (each folder has its own model)')
        model = get_models(args).cuda()

        #print("Ben33 hidden_dim:", model.hidden_dim)
        #print("Ben33 dropout:", model.dropout)


        #print("ben33",model)                # What is model?
        print(f'Ben34 model: {model}, args: {args}')
        print("ben35",model.model)          # Does it have `.model`?
        print(f'Ben36 model: {model.model}, args: {args}')

        #print("ben33",model.model.model_name)  # Only if the above exists

        #print(f'Ben32 model: {model.model.model_name}, args: {args}')
        ...
        if args.lr_adjust == 'case1':
            ...
            print ('Ben06 set different learning rates for different layers')
        ...
        print('Ben01 args before start  training: ', args)
        ...
        for epoch in range(args.epochs):
            ...
            print('Ben01 args before saving  training: ', args)
        ...
        print (f'Ben05>>>>> Finish: training on the {ii+1}-th folder, best_index: {best_index}, duration: {duration} >>>>>')
        # clear memory
        del model
        del optimizer
    ...
    #Bentzi add to avid not allowed chars in file name
    save_path = clean_filename(save_path)    
    print (f'Ben06 save results in {save_path}')
    ...
    for jj in range(len(test_loaders)):
        ...
        #Bentzi add to avid not allowed chars in file name
        save_path = clean_filename(save_path)        
        print (f' Ben07 save results in {save_path}')
        np.savez_compressed(save_path,
                            emo_probs=emo_probs,
                            args=np.array(args, dtype=object))
